<?php include("_includes/header.php"); ?> 


  
  <div id="otherContent">
  	<div class="holder">

		<div class="threeCol">
        	<h2>BORROW WITH CONFIDENCE, GET A PMG LOAN TODAY</h2>
            
            <img class="loanPic" src="_images/loan-application.jpg" alt="Loan Application" />
            
            <div class="right">
                <p>Click the buttons below to download your application forms to start your relationship with Progressive. Your best financial decision is just a click away.</p>
                
                <div class="buttonHolder">
                	<a href="_docs/Salary Deduction Form.pdf">Salary Deduction Form</a>
                        <br>
                        <br>
                        <br>
                        <a href="_docs/pmg-loan-form.pdf" target="_blank">Loan Application Form</a>
                        <br>
                        <br>
                        <br>
                        <a href="apply_now.php">Online Application</a>
                </div><!--end of . buttonHolder-->
                
            <!--    <form method="get" action="_docs/pmg-loan-form.pdf">
                	<button type="submit">Loan Form</button>
                </form> -->
                
                <p>To reduce our customer's efforts, completed forms may be sent to our offices as attachments. The email address is: <a href="">loans.pmg@gmail.com</a></p>
            </div><!--end of . right-->
            
            
        </div><!--end of .twoCol-->

		<?php include("_includes/sidebar.php"); ?> 
        
    </div><!--end of . holder-->
  </div><!--end of #mainContent-->
  <div id="bottomBanner">
  	<div class="holder">
        <p>call for more information</p>
        <b><a href="#">876-619-2965</a> / <a href="#">876-818-7645</a></b>
        <p>MON-FRI 09:00am to 05:00pm</p>
    </div><!--end of .holder-->
  </div><!--end of #bottomBanner-->
  
  <?php include("_includes/footer.php"); ?> 
